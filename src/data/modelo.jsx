        ...
        <div className="max-w-7xl mx-auto">
          ...
          {abaAtiva === 'executivo' && (
            <div className="space-y-6 animate-in fade-in duration-700">
              {/* RESUMO EXECUTIVO - COMANDO CENTRAL */}
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <p className="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-bold">Operação em Tempo Real</p>
                      </div>
                      <h2 className="text-3xl font-black text-white tracking-tight">Painel Executivo</h2>
                      <p className="text-sm text-slate-400 mt-1 font-medium">Consolidado Industrial • {new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 min-w-[450px]">
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-sm">
                      <p className="text-[10px] uppercase font-bold text-slate-500 mb-1">Presença Hoje</p>
                      <div className="flex items-end gap-2">
                        <span className="text-2xl font-bold text-emerald-400">{resumoFaltas.percentualPresenca.toFixed(1)}%</span>
                        <span className="text-[10px] text-slate-500 mb-1">da meta</span>
                      </div>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-sm">
                      <p className="text-[10px] uppercase font-bold text-slate-500 mb-1">Dias Ativos</p>
                      <div className="flex items-end gap-2">
                        <span className="text-2xl font-bold text-amber-400">{faturamentoAtual.diasAtivos}</span>
                        <span className="text-[10px] text-slate-500 mb-1">dias úteis</span>
                      </div>
                    </div>
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-sm">
                      <p className="text-[10px] uppercase font-bold text-slate-500 mb-1">Faturamento Mês</p>
                      <div className="flex items-end gap-2">
                        <span className="text-xl font-bold text-blue-400">R$ {faturamentoAtual.total.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                {[
                  { label: 'Faturamento Total', valor: formatarMoeda(faturamentoAtual.total), sub: 'Consolidado mensal', icon: DollarSign, color: 'text-blue-500' },
                  { label: 'Média por Dia', valor: formatarMoeda(faturamentoAtual.diasAtivos > 0 ? faturamentoAtual.total / faturamentoAtual.diasAtivos : 0), sub: 'Performance diária', icon: TrendingUp, color: 'text-emerald-500' },
                  { label: 'Ticket Médio', valor: formatarMoeda(faturamentoAtual.ticketMedio), sub: 'Valor por pedido', icon: ShoppingCart, color: 'text-purple-500' },
                  { label: 'Clientes Ativos', valor: faturamentoAtual.clientesAtivos, sub: 'Carteira no mês', icon: Users, color: 'text-amber-500' },
                  { label: 'Faltas Hoje', valor: resumoFaltas.ausentes, sub: 'Atenção operacional', icon: UserX, color: 'text-rose-500', alert: resumoFaltas.ausentes > 5 },
                  { label: 'Férias Hoje', valor: resumoFaltas.porTipo['Ferias'] || 0, sub: 'Planejamento RH', icon: CalendarIcon, color: 'text-slate-400' },
                ].map((kpi, i) => (
                  <div key={i} className={`bg-white border border-slate-200 rounded-2xl p-5 shadow-sm transition-all hover:shadow-md group ${kpi.alert ? 'ring-2 ring-rose-500/20 border-rose-200' : ''}`}>
                    <div className="flex justify-between items-start mb-3">
                      <div className={`p-2 rounded-xl bg-slate-50 ${kpi.color} group-hover:scale-110 transition-transform`}>
                        <kpi.icon size={18} />
                      </div>
                      {kpi.alert && <span className="flex h-2 w-2 rounded-full bg-rose-500" />}
                    </div>
                    <p className="text-[10px] uppercase tracking-wider text-slate-500 font-black">{kpi.label}</p>
                    <p className={`text-xl font-bold mt-1 tracking-tight ${kpi.alert ? 'text-rose-600' : 'text-slate-900'}`}>
                      {kpi.valor}
                    </p>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                ...
                <div className="xl:col-span-8 space-y-6">
                  <div className="bg-white border border-slate-200 rounded-3xl p-7 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                      ...
                      <div className="flex gap-2">
                        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-50 border border-slate-100 text-[10px] font-bold text-slate-500">
                          <div className="w-1.5 h-1.5 rounded-full bg-blue-600" /> Realizado
                        </div>
                      </div>
                    </div>
                    ...
                    {faturamentoAtual.porDia.length === 0 ? (
                      <div className="h-48 flex items-center justify-center border-2 border-dashed border-slate-100 rounded-2xl">
                        <p className="text-slate-400 text-xs font-medium italic">Aguardando dados do ERP...</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 gap-4">
                        {faturamentoAtual.porDia.slice(-8).map((item) => {
                          const maxDia = Math.max(...faturamentoAtual.porDia.map((d) => d.valor), 1);
                          const perc = (item.valor / maxDia) * 100;
                          const isHigh = item.valor > (faturamentoAtual.total / faturamentoAtual.diasAtivos);
                          
                          return (
                            <div key={item.dia} className="group">
                              <div className="flex items-center justify-between text-[11px] mb-1.5 px-1">
                                <span className="font-bold text-slate-600 group-hover:text-blue-600 transition-colors">{item.dia}</span>
                                <div className="flex items-center gap-2">
                                  {isHigh && <ArrowUpRight size={12} className="text-emerald-500" />}
                                  <span className="font-black text-slate-900">R$ {item.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                </div>
                              </div>
                              <div className="h-3 rounded-full bg-slate-50 overflow-hidden border border-slate-100 p-0.5">
                                <div 
                                  className={`h-full rounded-full transition-all duration-1000 ${isHigh ? 'bg-gradient-to-r from-blue-500 to-blue-600' : 'bg-slate-300'}`} 
                                  style={{ width: `${perc}%` }} 
                                />
                              </div>
                            </div>
                  ...
                  {/* ALERTAS OPERACIONAIS - ABSENTEÍSMO */}
                  <div className="bg-white border border-slate-200 rounded-3xl p-7 shadow-sm">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <span className="px-3 py-1 rounded-full bg-rose-50 text-rose-600 text-[10px] font-black uppercase border border-rose-100">
                        Crítico > 3 faltas
                      </span>
                    </div>
                    ...
                    {listaSetores.length > 0 ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                        {listaSetores.map((setor) => {
                          ...
                          const max = resumoMesAtualSetores.maxSetor || 1;
                          const perc = (valor / max) * 100;
                          const isCritical = valor > 3;
                          
                          return (
                            <div key={setor} className="py-2 border-b border-slate-50 last:border-0">
                              <div className="flex justify-between items-center mb-1.5">
                                <span className="text-[11px] font-bold text-slate-600 truncate max-w-[150px]">{setor}</span>
                                <span className={`text-[11px] font-black ${isCritical ? 'text-rose-600' : 'text-slate-900'}`}>{valor} faltas</span>
                              </div>
                      ...
                      <p className="text-slate-400 text-xs italic text-center py-8">Sem dados operacionais registrados.</p>
                    )}
                  </div>
                </div>

                {/* COLUNA LATERAL - RANKING E MÃO DE OBRA */}
                <div className="xl:col-span-4 space-y-6">
                  ...
                  <div className="bg-white border border-slate-200 rounded-3xl p-7 shadow-sm">
                    ...
                    {faturamentoAtual.topClientes.length === 0 ? (
                      <p className="text-slate-400 text-xs italic">Sem dados de faturamento.</p>
                    ) : (
                      <div className="space-y-5">
                        {faturamentoAtual.topClientes.slice(0, 5).map((item, idx) => {
                          const totalFat = faturamentoAtual.total || 1;
                          const share = (item.valor / totalFat) * 100;
                          
                          return (
                            <div key={item.cliente} className="relative">
                              <div className="flex items-start justify-between mb-1">
                                <div className="flex gap-3">
                                  <span className="text-xs font-black text-slate-300 mt-0.5">0{idx + 1}</span>
                                  <div>
                                    <p className="text-[11px] font-black text-slate-800 leading-tight uppercase">
                                      {item.info?.nome || item.cliente}
                                    </p>
                                    <p className="text-[9px] text-slate-400 font-bold mt-0.5">
                                      {item.info?.municipio ? `${item.info.municipio} / ${item.info.estado}` : 'Localização não informada'}
                                    </p>
                                  </div>
                                </div>
                                <div className="text-right">
                                  <p className="text-[11px] font-black text-emerald-600">R$ {item.valor.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</p>
                                  <p className="text-[9px] text-slate-400 font-bold">{share.toFixed(1)}% share</p>
                                </div>
                              </div>
                              <div className="h-1 w-full bg-slate-50 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500/30" style={{ width: `${share * 5}%` }} />
                              </div>
                            </div>
                          );
                        })}
                    ...
                    <button 
                      ...
                      className="w-full mt-6 py-3 rounded-xl border border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-colors"
                    >
                  ...
                  {/* STATUS MÃO DE OBRA */}
                  <div className="bg-white border border-slate-200 rounded-3xl p-7 shadow-sm overflow-hidden relative">
                    <div className="absolute top-0 right-0 p-4 opacity-5">
                      <Factory size={80} />
                    </div>
                    <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest mb-6">Mão de Obra</h3>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-4 rounded-2xl bg-emerald-50 border border-emerald-100">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[11px] font-black text-emerald-900 uppercase">Presentes</span>
                        </div>
                        <span className="text-xl font-black text-emerald-700">{resumoFaltas.presentes}</span>
                      </div>

                      <div className="flex items-center justify-between p-4 rounded-2xl bg-rose-50 border border-rose-100">
                        <div className="flex items-center gap-3">
                          <div className="p-2 rounded-lg bg-white text-rose-600 shadow-sm">
                            <UserX size={16} />
                          </div>
                          <span className="text-[11px] font-black text-rose-900 uppercase">Ausentes</span>
                        </div>
                        <span className="text-xl font-black text-rose-700">{resumoFaltas.ausentes}</span>
                      </div>

                      <div className="flex items-center justify-between p-4 rounded-2xl bg-amber-50 border border-amber-100">
                        <div className="flex items-center gap-3">
                          <div className="p-2 rounded-lg bg-white text-amber-600 shadow-sm">
                            <CalendarIcon size={16} />
                          </div>
                          <span className="text-[11px] font-black text-amber-900 uppercase">Em Férias</span>
                        </div>
                        <span className="text-xl font-black text-amber-700">{resumoFaltas.porTipo['Ferias'] || 0}</span>
                      </div>
                    </div>

                    <div className="mt-6 pt-6 border-t border-slate-100 flex justify-between items-center">
                      <span className="text-[10px] font-bold text-slate-400 uppercase">Total Colaboradores</span>
                      <span className="text-sm font-black text-slate-700">{resumoFaltas.total}</span>
                    </div>
                  </div>
                </div>
              </div>
